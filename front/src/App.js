...
function App() {
  ...
  const fileRef = useRef(); // reference to the native element of the input
  ...
  const getAllContacts = async (page=0, size =10)=>{
    try {
      ...
      setData(data);
      console.log(data);//console.log({data}); just the object
    } catch (error) {
      console.log(error);
    }
  }
  ...
  //once when this component renders
  useEffect(() => {
    getAllContacts();
  },[]);
  ...
  return (
    ...
    <Header toggleModal={toggleModal} nOfContacts={data.totalElements} />
    <main className='main' >
